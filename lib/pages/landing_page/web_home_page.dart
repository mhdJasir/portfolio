...
import 'package:portfolio/widgets/moving_text_widget.dart';
...
class _WebHomeState extends State<WebHome> {
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _topNavigator(1, "About", bloc.aboutMeKey),
                      _topNavigator(2, "Experience", bloc.experienceKey),
                      _topNavigator(3, "Work", bloc.workKey),
                      _topNavigator(4, "Contact", bloc.contactKey),
                    ],
                  ),
                ],
              ),
              ...
              children: [
                ...
                Flexible(
                  child: Text(
                    model.techs?.first ?? '',
                    style:
                        theme.titleMedium?.copyWith(color: AppColors.neonColor),
                  ),
                ),
              ],
            )
  ...
  Widget _topNavigator(int index, title, GlobalKey key) {
    return BlocBuilder<LandingCubit, LandingState>(
      builder: (context, state) {
        return InkWell(
          onTap: () {
            bloc.scrollToWidget(key.currentContext);
          },
          child: Row(
            children: [
              Text(
              ...
              sbw(10),
              MovingTextWidget(builder: (controller) {
                return HoverWidget(
                  builder: (isHovering) {
                    if (isHovering && !controller.isAnimating) {
                      controller.forward();
                    } else if(!isHovering && !controller.isAnimating){
                      controller.reverse();
                    }
                    return Text(
                      '$title',
